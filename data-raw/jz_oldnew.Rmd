...
library(deSolve)
DATA_DIR <- here::here()
WRITE_TO <- file.path(DATA_DIR, "tests", "testthat")
```
...
out <- data.table::melt(data.table::as.data.table(output),
                        measure.vars = c("POM", "MOM", "QOM", "DOM", "MB", "EP", "EM", "IC", "Tot"),
                        variable.name = "variable",
                        value.name = 'value')
out$units <- 'mg C/g soil'
out$variable <- gsub(out$variable, pattern =  "OM", replacement = "")
out$variable <- gsub(out$variable, pattern =  "MB", replacement = "B")

write.csv(out, file = file.path(WRITE_TO, "mend_jz.csv"), row.names = FALSE)
```
...
write.csv(out, file = file.path(WRITE_TO, "COM_jz.csv"), row.names = FALSE)
```
...
write.csv(out, file = file.path(WRITE_TO, "LIN_jz.csv"), row.names = FALSE)
```
...
write.csv(out, file = file.path(WRITE_TO, "TOY_jz.csv"), row.names = FALSE)
```
