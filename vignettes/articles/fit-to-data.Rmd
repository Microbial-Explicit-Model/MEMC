...
mod1  <- configure_model(name = "MM DOMdecomp", params = MEMC::default_params, state = state)

# Fit the the "maximum specific decomposition" parameters. 
fit1 <- memc_modfit(config = mod1, comp_data = comp_data, x =c(V.d=2, V.p=5, V.m=0.01))
```
...
```{r}
out1 <- solve_model(mod1,  0:max(comp_data$time), params = fit1$par)[["results"]]
...
```{r}
out2 <- solve_model(mod2,  0:max(comp_data$time), params = fit2$par)[["results"]]
...
```{r}
default1 <- solve_model(mod1, 0:max(comp_data$time))[["results"]]
default2 <- solve_model(mod2, 0:max(comp_data$time))[["results"]]
