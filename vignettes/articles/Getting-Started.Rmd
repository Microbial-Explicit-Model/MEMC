...
After following the [installation instructions](Install-Build.html) you should be ready to follow along with this tutorial where we will walk thorough basic MEMC run, change parameter values, initial conditions, and change model dynamics. 
...
new_initial_cond[["B"]] <- new_initial_cond[["B"]] * 2
...
out2$results$name <- "Double B"
```

```{r}
to_plot <- rbind(out1$results, out2$results) 
...
```{r}
out3 <- solve_model(mod, time, params = c("V.d" = 10))
out3$results$name <- "Increase V.d"
```

Visualize model runs. 

```{r}
to_plot <- rbind(out1$results, out3$results) 
