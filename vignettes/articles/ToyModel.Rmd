...
The `memc_configure` function allows users to build their own SOM model. All of the MEMC models use the same SOM pool structure but users will be able to decide which of the formulations (LM, MM, RMM, ECA, or DD) are used for POC decomposition (1), DOC uptake (2), and microbial biomass decay (3). The following example will demonstrate how changing the DOC uptake flux impacts the model projections. 
...
Now switch the DOC uptake to use the RMM dynamics. 
...
Now try setting the DOC uptake to ECA dynamics. 
