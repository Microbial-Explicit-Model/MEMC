...
format_out <- function(df) {
  ...
  names <- c("POC", "MOC", "QOC", "DOC", "MB", "EP", "EM", "IC", "Tot")
...
solveMEND <- function(pars,times){
 derivs <- function(t, state, pars){
   ...
   # Carbon use effciency
        kD = 0.25
        kP = 50 #half saturation constant for POC
        kM = 250
        Kads = 0.006 
        Kdes = 0.001 
        Qmax = 3.4
    ...
    F1 <- Vd* MB *DOC /( kD + DOC)  ## microbial uptake 
    F2 <- Vp* EP * POC / (kP + POC) # POC decomposition
    F3 <- Vm * EM * MOC / (kM + MOC) #MOAM decomposition
    F6 <- Kads * DOC *(1- QOC/ Qmax) ##adsorption
    F7 <- Kdes * QOC/Qmax  ##desorption
    ...
    dPOC <- (1 - gD) * F8- F2+Input
    dMOC <- (1 - fD) * F2 - F3+Input
    dMB <- F1*CUE - F8 - (F9em + F9ep) # Microbial growth= microbial uptake *CUE
    dDOC <- fD * F2 + gD * F8 + F3 + (F10em + F10ep)- F1 - (F6 - F7)
    dQOC <- F6 - F7
    dEP <- F9ep - F10ep
    dEM <- F9em - F10em
    dIC <- F1*(1-CUE)   #CO2 fllux
    dTot <- -F1*(1-CUE)+Input*2
    return(list(c(dPOC, dMOC, dQOC, dDOC, dMB,dEP,dEM, dIC, dTot)))
      })
 }


#state<-c(POC=3.25, MOC=27.97, QOC=0, DOC=0.21, MB=0.64, EP=0.064, EM=0.064, IC=0, Tot=32.198)#Mollisol
#state<-c(POC=4.25, MOC=11.04, QOC=0, DOC=0.17, MB=0.05, EP=0.005, EM=0.005, IC=0, Tot=15.52)#Gelisol
state<-c(POC=4.71, MOC=17.67, QOC=0, DOC=0.148, MB=0.82, EP=0.0082, EM=0.0082, IC=0, Tot=23.484)#Ultisol
#state<-c(POC=20.06, MOC=64.86, QOC=0, DOC=0.64, MB=0.86, EP=0.086, EM=0.086, IC=0, Tot=86.592)#Andisol
...
solveCOM <- function(pars,times){
 derivs <- function(t, state, pars){
   ...
   # Carbon use effciency
        ...
        Input=0 #input per day
    F1 <- Vd* MB *DOC /( kD + DOC)  ## microbial uptake 
    F2 <- Vp* EP* POC / (kP + EP) # POC decomposition
    F3 <- Vm * EM * MOC / (kM + MOC) #MOAM decomposition
    F6 <- Kads * DOC *(1- QOC/ Qmax) ##adsorption
    F7 <- Kdes * QOC/Qmax  ##desorption
...
#state<-c(POC=3.25, MOC=27.97, QOC=0, DOC=0.21, MB=0.64, EP=0.064, EM=0.064, IC=0, Tot=32.198)#Mollisol
#state<-c(POC=4.25, MOC=11.04, QOC=0, DOC=0.17, MB=0.05, EP=0.005, EM=0.005, IC=0, Tot=15.52)#Gelisol
state<-c(POC=4.71, MOC=17.67, QOC=0, DOC=0.148, MB=0.52, EP=0.052, EM=0.052, IC=0, Tot=23.484)#Ultisol
#state<-c(POC=20.06, MOC=64.86, QOC=0, DOC=0.64, MB=0.86, EP=0.086, EM=0.086, IC=0, Tot=86.592)#Andisol
...
##CORPSE model (uses linear kinetics for POC decomposition)
```{r linear}
...
solveLIN<- function(pars,times){
 derivs <- function(t, state, pars){
  ...
  kD = 0.25
        ...
        CUE=0.4
        Input=0 #input per day
    F1 <- Vd* MB *DOC /( kD + MB)  ## microbial uptake 
    F2 <- Vp* POC # POC decomposition
    F3 <- Vm  * EM * MOC / (kM + MOC) #MOAM decomposition
    F6 <- Kads * DOC *(1- QOC/ Qmax) ##adsorption
    F7 <- Kdes * QOC/Qmax  ##desorption
    F8 <- (1- pEP- pEM)* MB *0.4*Vd *MB #microbial biomass decay (biomass decay rate=0.2*growth rate)
    ...
    dMOC <- (1 - fD) * F2 - F3+Input
    dMB <- F1*CUE - F8 - (F9em + F9ep) # Microbial growth= microbial uptake *CUE
    dDOC <- fD * F2 + gD * F8 + F3 + (F10em + F10ep)- F1 - (F6 - F7)+Input
    dQOC <- F6 - F7
    dEP <- F9ep - F10ep
    dEM <- F9em - F10em
    dIC <- F1*(1-CUE)   #CO2 fllux
...
###toy model (uses MM and rMM for POC and DOC decomp, respectively)
```{r toy}
##solving simple MEND model
pars<- list (Vd=0.5,Vp=0.001, Vm=0.001)
...
colnames(mfit) <- c("time", "POC","MOC","QOC","DOC","MB","EP","EM","IC","Tot")
head(mfit)

write.csv(mfit, "Ul_POC_LIN.csv")

# pdf("Andisol_POC_LIN.pdf", width=6, height=4)
...
print(system.time(sR <- sensRange(func = solveLIN, times = 0:365,  parms = pars, dist ="norm",parMean=mean, parCovar=covar, sensvar = c("DOC", "POC","MOC","MB"), parRange = parRanges, num = 100)))
...
print(system.time(sR <- sensRange(func = solveCOM, times = 0:365, parms = pars, dist ="unif",parMean=mean, parCovar=covar, sensvar = c("DOC", "POC","MOC","MB"), parRange = parRanges, num = 100)))
...
# dev.off()
# 
# pdf("POC.pdf", width=6, height=4)
# par(xpd = T, mar = par()$mar + c(0,0,0,10))
plot(mfit1$time, mfit1$POC, xlab = "Time (d)", ylab = "POC (mgC g-1 soil)",type="l",lwd=2,col="#fdb863",xlim=c(0,365), ylim=c(0,5))
lines(mfit2$time, mfit2$POC, lwd=2, col="#e08214")+lines(mfit3$time, mfit3$POC, lwd=2, col="#b35806")
legend("bottomright",inset = c(- 0.6, 0), c("MEND","COMISSION","CORPSE"), lwd=2, lty=1,col=c("#fdb863","#e08214","#b35806"))
# dev.off()

# pdf("MOC.pdf", width=6, height=4)
# par(xpd = T, mar = par()$mar + c(0,0,0,10))
plot(mfit1$time, mfit1$MOC, xlab = "Time (d)", ylab = "MOC (mgC g-1 soil)",type="l",lwd=2,col="#fdb863",xlim=c(0,365), ylim=c(15,25))
lines(mfit2$time, mfit2$MOC, lwd=2, col="#e08214")+lines(mfit3$time, mfit3$MOC, lwd=2, col="#b35806")
legend("bottomright",inset = c(- 0.6, 0), c("MEND","COMISSION","CORPSE"), lwd=2, lty=1,col=c("#fdb863","#e08214","#b35806"))
# dev.off()
...
###with fresh C input
# pdf("pct_MOC_input05.pdf", width=6, height=4)
# par(xpd = T, mar = par()$mar + c(0,0,0,10))
plot(mfit1$time, mfit1$MOC/mfit1$Tot, xlab = "Time (d)", ylab = "MOC%",type="l",lwd=2,col="#fdb863",xlim=c(0,3650), ylim=c(0,1))
# lines(mfit2$time, mfit2$MOC/mfit2$Tot, lwd=2, col="#e08214")+lines(mfit3$time, mfit3$MOC/mfit3$Tot, lwd=2, col="#b35806")
# legend("bottomright",inset = c(- 0.6, 0), c("MEND","COMISSION","CORPSE"), lwd=2, lty=1,col=c("#fdb863","#e08214","#b35806"))
# dev.off()
