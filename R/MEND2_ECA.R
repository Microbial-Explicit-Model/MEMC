#' Define the 2 P pool MEND fluxes that use ECA for microbial biomass uptake #TODO figure out what ECA stands for
#'
#' \code{MEND2_ECA_fluxes} Defines a system of equations that
...
#' reverse Michaelis–Menten kinetics in microbial doc uptake.
#' @family MEND2_ECA
#' @family carbon flux functions
#' @importFrom assertthat assert_that has_name
#' @noRd
MEND2_ECA_fluxes  <- function(state, parms){
...
#'
#' # Solve MEND2_ECA
#' out <- MEND2_ECA(parameters = p, time = t,  inital_state = state)
#' @export
MEND2_ECA <- function(parameters, time, inital_state, method = NULL){
  ...
  out <- solver(params = parameters,
                ...
                carbon_fluxes_func = MEND2_ECA_fluxes, method = method)
