#' Define the 2 P pool MEND fluxes with reverse Michaelis–Menten kinetics
#'
#' \code{MEND2_RM_fluxes} Defines a system of equations that
...
#' @noRd
MEND2_RM_fluxes  <- function(state, parms){
...
#' Solve 2 P pool MEND with reverse michaelis menten kinetics
#'
...
#'
#' # Solve MEND2_RM
#' out <- MEND2_RM(parameters = p, time = t,  inital_state = state)
#' @export
MEND2_RM <- function(parameters, time, inital_state, method = NULL){
  ...
  out <- solver(params = parameters,
                ...
                carbon_fluxes_func = MEND2_RM_fluxes, method = method)
