...
carbon_pools <-function(t, env, state = NULL, params = NULL,
  ...
  with(as.list(env), {
    ...
    dP <- (1 - g.d) * fluxes$F8() - fluxes$F2() + Input
    # M = mineral-associated organic carbon (MOC)
    dM <- (1 - f.d) * fluxes$F2() - fluxes$F3() + Input
    # Q = active layer of MOC
    dQ <- fluxes$F6() - fluxes$F7()
    # B = microbial biomass carbon
    dB <- fluxes$F1() * CUE - fluxes$F8() - (fluxes$F9.ep() + fluxes$F9.em())
    # D = dissolved organic carbon
    dD <- f.d * fluxes$F2() + g.d * fluxes$F8() + fluxes$F3() + (fluxes$F10.em() + fluxes$F10.ep()) - fluxes$F1() - (fluxes$F6() - fluxes$F7())
