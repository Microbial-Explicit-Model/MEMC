#' Define the 2 P pool MEND fluxes that uses the ... TODO figure out what millen stands for
#'
#' \code{MEND2_MILLEN_fluxes} Defines a system of equations that
...
#' @return A list of functions that calculate the fluxes between the MEND 2 carbon pools that uses
#' TODO doc uptake.
#' @family MEND2_MILLEN
#' @family carbon flux functions
#' @importFrom assertthat assert_that has_name
#' @noRd
MEND2_MILLEN_fluxes  <- function(state, parms){
  ...
  with(as.list(c(state, p)), {
    ...
    fxn_list <- list(
      "F1d1" = function(){
        ...
        V.d1 * B * D1 /( K.d1 + B) / ( K.d1 + D1)
      },
      "F1d2" = function(){
        # Microbial uptake from D2 pool
        V.d2 * B * D2 /( K.d2 + B) /( K.d2 + D2)
      },
      "F2p1" = function(){
      ...
      "F8" = function(){
        ...
        (1 - p.ep - p.ep - p.em) * V.d1 * E.c1 * B * 0.5
      },
      "F9.ep1" = function(){
        # Enzyme production
...
#' Solve 2 P pool MEND with ECA kinetics
#'
#' \code{MEND2_ECA} Run and solve 2 P pool MEND  \href{https://doi.org/10.1890/12-0681.1}{Wang et al. 2013} that
#' uses ECA kinetics in microbial uptake of DOC.
#'
...
#'
#' # Solve MEND2_MILLEN
#' out <- MEND2_MILLEN(parameters = p, time = t,  inital_state = state)
#' @export
MEND2_MILLEN <- function(parameters, time, inital_state, method = NULL){
  ...
  out <- solver(params = parameters,
                ...
                carbon_fluxes_func = MEND2_MILLEN_fluxes, method = method)
