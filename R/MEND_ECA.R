#' Define MEND fluxes that uses ECA dynamics
#'
#' \code{MEND_ECA_fluxes} Defines a system of equations that
...
#' represenation of the DOC uptake by microbial biomass. Here this dynamic is driven by TODO
#'
...
#' @noRd
MEND_ECA_fluxes <- function(state, parms){
...
#' Solve MEND with ECA kinetics
#'
#' \code{MEND_ECA} Run and solve MEND 2013  \href{https://doi.org/10.1890/12-0681.1}{Wang et al. 2013} that
...
#'
#' # Solve MEND_ECA
#' out <- MEND_ECA(parameters = p, time = t,  inital_state = state)
#' @export
MEND_ECA <- function(parameters, time, inital_state){
  ...
  out <- solver(params = parameters,
                ...
                carbon_fluxes_func = MEND_ECA_fluxes)
